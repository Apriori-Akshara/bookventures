...
const slider = ({fiction}) => {
  let reversed = fiction.sort((a, b) => b.id - a.id);
  const key = 0;
  ...
  return (
    <>
     ...
     <div key={key} className={styles.bvcard}>
      <Link className={styles.link} href={'/NoblePrizeWinners'} >
      <div className={styles.slide}>
        <div className={styles.bvimgcont}>
          <Image src={'/homepage/awards/noble.jpeg'} width={400} height={400} unoptimized={true} alt="Book Image" />
        </div>
        <div className={styles.bvcardinfo}>
          <h2>Noble Prize Winners 2013-23</h2>
        </div>
      </div>
        </Link>
      </div>
