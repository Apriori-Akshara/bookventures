...
export default function ProfileComponent({ onClosepro }) {
  ...
  const handlesendData = async (e) => {
    ...
    const responseData = await fetch(
      `${process.env.NEXT_PUBLIC_URL}/users/${user.user.id}`,
      {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          ...
          password: updateuser.npassword
            ? updateuser.npassword
            : user.user.npassword,
        }),
      }
    );
    ...
    newprofiledata.username &&
      cookie.set("username", newprofiledata.username, { expires: 1 / 24 });
    newprofiledata.email &&
      cookie.set("email", newprofiledata.email, { expires: 1 / 24 });
    newprofiledata.img &&
      cookie.set("img", newprofiledata.img, { expires: 1 / 24 });
    newprofiledata.jwt &&
      cookie.set("jwt", newprofiledata.jwt, { expires: 1 / 24 });
    newprofiledata.id &&
      cookie.set("id", newprofiledata.id, { expires: 1 / 24 });
    dispatch(falsch());
    dispatch(
      Signinsuccess({
    ...
    <div className={styles.wcontainer}>
      <div className={styles.innercontainer}>
        <div className={styles.topsection}>
          ...
          <div className={styles.close} onClick={onClosepro}>
            X
          </div>
        </div>
        {update ? (
          <div>
            <div className={styles.middle}>
              Press down the button below to LogOut or
              <br /> Update Userinfo!
            </div>
            <div className={styles.btnflex}>
              <button onClick={deletecookie} className={styles.logoutbtn}>
                LogOut
              </button>
              <button
          ...
          <form className={styles.form} onSubmit={handlesendData}>
            <div className={styles.formflex}>
              <div className={styles.labelflex}>
                ...
                <div className={styles.info}>{user.user.username}</div>
              </div>
              <input
                type="text"
       ...
       onChange={(e) => setUpdateuser({...updateuser, cpassword: e.target.value })}
            ...
            <div className={styles.formflex}>
              ...
              <input
                ...
                onChange={(e) =>
                  setUpdateuser({ ...updateuser, npassword: e.target.value })
                }
              />
            </div>
            <div className={styles.formflex}>
