...
const Index = () => {
  ...
  try{
    const res = await axios.get(process.env.NEXT_PUBLIC_URL+`/books?filters[name][$startsWithi]=${starts}&populate=*`, {
     headers: {
  ...
  return (
    <div>
      ...
      <div className={styles.searchcontainer}>
        <input placeholder='search by writing the name' value={starts} onChange={e => setStarts(e.target.value)} />
        {/* <button><BsSearch /></button> */}
      </div>
      <div className={styles.cards}>
        {product.reverse().slice(0,8).map(data=>{
